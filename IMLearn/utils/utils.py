...
def split_train_test(X: pd.DataFrame, y: pd.Series,
    ...
    n_features) Design matrix of train set

    train_y : Series of shape (ceil(train_proportion * n_samples), )
        Responses of training samples

    test_X : DataFrame of shape (floor((1-train_proportion) * n_samples),
    n_features) Design matrix of test set

    test_y : Series of shape (floor((1-train_proportion) * n_samples), )
        Responses of test samples

    """
    train_sample_index = np.random.choice(X.index,
                                          size=math.ceil(train_proportion *
                                                         X.shape[0]),
                                          replace=False)
    train_x = X[X.index.isin(train_sample_index)]
    train_y = y[y.index.isin(train_sample_index)]
    test_x = X[~X.index.isin(train_sample_index)]
